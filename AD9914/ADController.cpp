...
void ADController::resetAllRegisters(char* filePath) {
	//cout << "Resetting all registers..." << endl;
	for (int i = 0 ; i < 2 ; i++) { // TODO find registry that should ready this for writing!
		SetPortValue(0, 0x7);
		Sleep(10);
		SetPortValue(0, 0x3);
...
void ADController::getBackup(char* filePath) {
	ofstream wFile;
	wFile.open(filePath);
